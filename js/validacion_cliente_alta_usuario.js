// Funciones de validación
	...
	function passwordValidation(){
		...
		valid = valid && (hasNumber.test(pwd))
			&& (hasUpperCases.test(pwd))
			&& (hasLowerCases.test(pwd));
		
		// Si no cumple las restricciones, devolvemos un mensaje de error
		if(!valid){
			var error = "Por favor, introduzca una contraseña correcta. Longitud >= 8."
		}else{
			var error = "";
		}
	...
	function passwordConfirmation(){
		...
		var passconfirm = document.getElementById("cpass");
		var confirmation = document = passconfirm.value;
		
		// Los comparamos y devolvemos un mensaje de error en caso de que no sean iguales
		if(pwd!=confirmation) {
			var error = "Por favor introduzca la misma contraseña";
		} else {
			var error = "";
		}
